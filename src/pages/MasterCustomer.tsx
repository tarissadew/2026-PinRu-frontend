...
import { getBookings, deleteBooking, updateBooking } from "../services/api"; // Import fungsi baru
import { 
  MoreVertical, 
  Mail, 
  Calendar, 
...
export default function MasterCustomer() {
  ...
  const handleDelete = async (id: number, name: string) => {
    const confirmDelete = window.confirm(`Apakah Anda yakin ingin menghapus data transaksi ${name}?`);
    if (confirmDelete) {
      try {
  ...
  const handleEdit = async (id: number, currentName: string) => {
    const newName = prompt("Ubah Nama Pengguna:", currentName);
    if (newName && newName !== currentName) {
      try {
        await updateBooking(id, { customerName: newName });
    ...
    <DashboardLayout role="Admin">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {customerStats.map((c) => (
            <div key={c.name} className="bg-white border border-gray-100 rounded-4xl p-6 shadow-sm hover:shadow-xl transition-all relative group">
                ...
                <div className="relative group/menu">
                  ...
                  <div className="absolute right-0 top-full hidden group-hover/menu:block bg-white border border-gray-100 shadow-xl rounded-2xl py-2 w-32 z-10">
                    <button 
                      ...
                      className="w-full flex items-center gap-2 px-4 py-2 text-xs font-bold text-blue-600 hover:bg-blue-50"
                    >
                      <Edit3 size={14} /> Edit
                    </button>
                    <button 
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="w-14 h-14 bg-linear-to-br from-green-400 to-[#00D084] text-white rounded-2xl flex items-center justify-center text-xl font-black">
                  {c.name.charAt(0)}
                </div>
                <div>
                  <h4 className="font-extrabold text-gray-800 text-lg">{c.name}</h4>
              ...
              <div className="bg-[#F8F9FA] rounded-3xl p-5 border border-gray-100">
                ...
                <div className="w-full bg-gray-200 h-2 rounded-full overflow-hidden">
                  <div 
                    className="bg-[#00D084] h-full rounded-full transition-all duration-1000" 
              ...
              <div className="mt-6 flex justify-between items-center">
                <div className="flex gap-2">
                  ...
                  <span className="bg-gray-50 text-gray-400 px-3 py-1 rounded-lg text-[9px] font-black uppercase border border-gray-100">Active</span>
                </div>
                <p className="text-[10px] text-gray-300 font-bold italic">ID: {c.id}</p>
              </div>
            </div>
          ))}
        </div>
